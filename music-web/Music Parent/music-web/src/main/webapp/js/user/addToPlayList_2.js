...
function addToPlayList(obj) {
	...
	//播放列表ul
	var $ul = $("#g_playlist ul", window.parent.document);
	...
	$li.mouseover(function() {
		...
		if(!$("#myPlayer", window.parent.document)[0].paused) {//如果当前有歌曲在播放，则暂停该歌曲的播放
			window.parent.stopCurSong();
			$("#g_player .play .words", window.parent.document).attr("lang", songId);//设置刚播放完的歌曲的id
	...
	$li.find(".icn-del").click(function() {		
		$li.remove();
		$("#songCountInList", window.parent.document).text(parseInt($("#songCountInList", window.parent.document).text()) - 1);//更新播放列表中的歌曲数目
		$("#g_player .ctrl .icn-list", window.parent.document).text($("#songCountInList", window.parent.document).text());//更新播放列表歌曲数
	});

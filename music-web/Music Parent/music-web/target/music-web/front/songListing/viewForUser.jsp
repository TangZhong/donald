	...
	<script type="text/javascript">
		$(function() {
			...
			//点击收藏歌单
   			...
   			//点击删除按钮，从歌单中移除歌曲
   			$(".opt .u-icn-11").click(function() {
   				if(!confirm("确定从歌单中移除该歌曲？")) {
   					return;
   				}
   				//获取当前选择的歌曲的编号
   				var curSongId = $(this).parent().parent().attr("lang");
   				$.post('songListing_removeSong.action', {listingId:'${vo.id}', songId: curSongId}, function(data) {
   					if(data == 'success') {//歌单中的歌曲的移除
   						alert("歌单中的歌曲移除成功!");
   						window.open(window.parent.location.href, "_top");
   					} else {
   						alert("歌单中的歌曲移除失败!");
   					}
   				});
	...
	<div class="ui_layout">
		...
		<div class="list_info">
			...
			<div class="detail f-ff2">
				...
				<!-- 创建用户 -->
				<div class="user">
					<a href="user_info.action?id=${vo.userPo.id}" class="face" target="_top">
						<img width="35" height="35" src="${vo.userPo.headImg}">
					</a>
					<span class="name">
						<a class="s-fc7" href="user_info.action?id=${vo.userPo.id}" target="_top">${vo.userPo.account}</a>
					</span>
					...
					<td class="second_td">
						<div class="f-cb" lang="${song.id}">
							...
							<div class="opt" style="display: none;">
								...
								<span class="u-icn u-icn-11" title="删除">&nbsp;</span>
							</div>
						</div>
						</td>

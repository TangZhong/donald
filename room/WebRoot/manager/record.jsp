		...
		<div>
			<table class="table table-striped table-bordered table-hover datatable">
				...
				<c:forEach  varStatus="i" var="record" items="${recordList }">
					<tr>
						<td>${record.date }</td>
						<td>${record.unumber }</td>
						<td>${record.studentName }</td>
						<td>${record.dormBuildName==null?"æ ":record.dormBuildName }</td>
						<td>${record.dormName }</td>
						<td>${record.detail }</td>
						<td>
							<button class="btn btn-mini btn-danger" type="button" onclick="recordDelete(${record.recordId })">å é¤</button></td>
					</tr>
				</c:forEach>
				</tbody>
			</table>
